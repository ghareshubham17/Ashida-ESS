...
export default function LeaveApplicationList() {
  ...
  // Fetch Leave applications from API with pagination
  const fetchLeaveApplications = useCallback(async (pageNum: number = 0, isRefresh: boolean = false) => {
    try {
      if (pageNum === 0) {
      ...
      console.log(`Fetching Leave applications - Page: ${pageNum}, Status: ${selectedStatus}, Sort: ${sortOrder}`);
      ...
      console.log(`Fetched ${leaveApps.length} Leave Applications for page ${pageNum}`);
      ...
      if (isRefresh || pageNum === 0) {
        setLeaveApplications(transformedApps);
      } else {
        setLeaveApplications(prev => [...prev, ...transformedApps]);
      }

      // Check if there are more records
      setHasMore(leaveApps.length === PAGE_SIZE);
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await fetchLeaveApplications(0, true);
  ...
  const onLoadMore = useCallback(() => {
    ...
    setLoadingMore(true);
    fetchLeaveApplications(currentPage + 1, false);
  }, [loadingMore, hasMore, isLoadingApplications, currentPage, fetchLeaveApplications]);
      ...
      {item.leave_approver && (
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: theme.colors.textSecondary }]}>
            Leave Approver: {item.leave_approver}
          </Text>
        </View>
      )}
      ...
      <SafeAreaView style={styles.safeArea} edges={['bottom']}>
        ...
        <View style={[styles.addButtonContainer, { backgroundColor: theme.colors.card }]}>
          ...
          >
            ...
            <Text style={styles.addButtonText}>Add Leave Application</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={[styles.filtersContainer, { backgroundColor: theme.colors.card }]}>
                ...
                <View style={[styles.dropdown, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
                  {['All', 'Open', 'Approved', 'Rejected', 'Cancelled'].map(status => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        setSelectedStatus(status as 'All' | LeaveStatus);
          ...
          <View style={styles.actionRow}>
            ...
            <Text style={[styles.statsText, { color: theme.colors.textSecondary }]}>
              {leaveApplications.length} application{leaveApplications.length !== 1 ? 's' : ''}
            </Text>
          </View>
        </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptySubtext, { color: theme.colors.textSecondary }]}>
                Create your first Leave application using the button above
              </Text>
            </View>
          }
          ListFooterComponent={
        ...
        {applicationsError && (
          <View style={[styles.errorContainer, { backgroundColor: theme.colors.card }]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => fetchLeaveApplications(0, false)}
            >
